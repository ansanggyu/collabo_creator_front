...
export const getOfflineStoreList = async (creatorId: string): Promise<IOfflineStore[]> => {
    try {
        ...
        console.error("Failed to fetch stores by creator:", error.message);
...
export const registerOfflineStore = async (
    ...
    try {
        const result = await jwtAxios.post(`${host}/add`, storeData, {
            params: { creatorId },
...
export const updateOfflineStore = async (
    ...
    try {
        const response = await jwtAxios.put(`${host}/update/${storeNo}`, storeData, {
